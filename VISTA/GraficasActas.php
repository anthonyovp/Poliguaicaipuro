...
$(function () {
    $('#unico').highcharts({
        ...
        title: {
            text: 'Porcentaje de Actas por Año'
        },
        tooltip: {
        ...
        series: [{
            ...
            data: [
              ...
              <?php
                 $obj->conectar();
                 $sql="SELECT DISTINCT EXTRACT(YEAR FROM fecha_act) AS Orderyear FROM acta ORDER BY EXTRACT(YEAR FROM fecha_act) DESC LIMIT 10";
                 $datos=$obj->consulta_query($sql);
                 while($fila = mysqli_fetch_array($datos)){

                      $fecha=$fila['Orderyear'];
                      $sql2="SELECT * FROM acta where EXTRACT(YEAR FROM fecha_act)='$fecha'";
                       $total=$obj->cantidad($sql2);

               
              ?> 

              ['<?php  echo $fecha;?>',<?php echo $total?>],
