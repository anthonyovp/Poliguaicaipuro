 ...
 <section id="container" class="">
      ...
      <!-- Today status end -->
        ...
        <?php
                 $obj->conectar();
                 $fecha=$_POST['anio'];
                $sql="SELECT DISTINCT procedimiento.nombre_pro FROM procedimiento INNER JOIN acta ON acta.cod_pro=procedimiento.codigo_pro WHERE EXTRACT(YEAR FROM acta.fecha_act)='$fecha'";
               
                 $datos=$obj->cantidad($sql);
                  if($datos<1){
                      echo "<script language='javascript'> alert('No hay Procedimientos en este Año!'); window.location.href='GraficasActas.php';</script>"; 
                    } 
                   else{
...
$(function () {
    $('#unico3').highcharts({
        ...
        title: {
            text: 'Estadísticas de Procedimientos en el Año <?php echo $fecha?>'
        },
        yAxis: {
            min: 0,
            title: {
        ...
        <?php
                 ...
                 while($fila = mysqli_fetch_array($datos)){
                      $fecha=$fila['fechanue'];
*/
                      $sql2="SELECT DISTINCT procedimiento.codigo_pro FROM procedimiento INNER JOIN acta ON acta.cod_pro=procedimiento.codigo_pro WHERE EXTRACT(YEAR FROM acta.fecha_act)='$fecha'";
                      ...
                      while($fila2 = mysqli_fetch_array($datos2)){
                            ...
                            $nuepro=$fila2['codigo_pro'];
                               $sql3="SELECT procedimiento.nombre_pro FROM procedimiento INNER JOIN acta ON acta.cod_pro=procedimiento.codigo_pro WHERE EXTRACT(YEAR FROM acta.fecha_act)='$fecha' AND procedimiento.codigo_pro='$nuepro'";
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12">
          ...
          <form  action="GraficasActasSegunAnio.php" method="POST">        
            <div class="login-wrap">
                  ...
                  <table align="center">
                    <tr>
                      <td>
                          <center><input type="number" class="form-control" name="anio" placeholder="Año a Buscar" required autofocus> </center>
                      </td>
                      <td>
                         <center><button class="btn btn-primary " name="buscar" type="submit" >Buscar</button></center>                
                      </td>
                      <td>
                         <center><a href="GraficasActas.php" class="btn btn-primary ">Regresar</a></center>                
                      </td>
