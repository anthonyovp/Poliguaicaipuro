 ...
 <section id="container" class="">
      ...
      <!-- Today status end -->
        ...
        <?php
                 $obj->conectar();
                 $sector=$_POST['sector'];
                $sql="SELECT  procedimiento.nombre_pro FROM procedimiento INNER JOIN acta ON acta.cod_pro=procedimiento.codigo_pro WHERE acta.sector_act='$sector'";
               
                 $datos=$obj->cantidad($sql);
                  if($datos<1){
                      echo "<script language='javascript'> alert('No hay Procedimientos en este Sector!'); window.location.href='GraficasActasSector.php';</script>"; 
                    } 
                   else{
...
$(function () {
    $('#unico3').highcharts({
        ...
        xAxis: {
            categories: [
              <?php
                 ...
                 $sql="SELECT DISTINCT procedimiento.nombre_pro FROM procedimiento INNER JOIN acta ON acta.cod_pro=procedimiento.codigo_pro WHERE acta.sector_act='$sector'";
        ...
        title: {
            text: 'Estadísticas de Procedimientos en el Sector <?php echo $sector?>'
        },
        yAxis: {
            min: 0,
            title: {
                text: 'Cantidad de Actas por Procedimiento',
                align: 'high'
            },
            labels: {
                overflow: 'justify'
        ...
        <?php
                 ...
                 /*$sql="SELECT DISTINCT EXTRACT(YEAR FROM fecha_act) AS fechanue FROM acta ORDER BY  EXTRACT(YEAR FROM fecha_act)  ASC LIMIT 10";
                 $datos=$obj->consulta_query($sql);
                 while($fila = mysqli_fetch_array($datos)){
                      $fecha=$fila['fechanue'];
*/
                      $sql2="SELECT DISTINCT procedimiento.codigo_pro FROM procedimiento INNER JOIN acta ON acta.cod_pro=procedimiento.codigo_pro WHERE acta.sector_act='$sector'";
                      $datos2=$obj->consulta_query($sql2);
                      ?>
                      { name: ['<?php  echo $sector?>'],
                          data: [
                      <?php
                      while($fila2 = mysqli_fetch_array($datos2)){
                      
                            $nuepro=$fila2['codigo_pro'];
                               $sql3="SELECT procedimiento.nombre_pro FROM procedimiento INNER JOIN acta ON acta.cod_pro=procedimiento.codigo_pro WHERE acta.sector_act='$sector' AND procedimiento.codigo_pro='$nuepro'";

                               $datos3=$obj->cantidad($sql3);
    ...
    <div class="row">
        <div class="col-lg-12 col-md-12">
          ...
          <form  action="GraficasActasSegunSector.php" method="POST">        
            <div class="login-wrap">
                  ...
                  <table align="center">
                    <tr>
                      <td>
                          <center><input type="text" class="form-control" name="sector" placeholder="Sector a Buscar" required autofocus> </center>
                      </td>
                      <td>
