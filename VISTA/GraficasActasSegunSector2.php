 ...
 <section id="container" class="">
      ...
      <!-- Today status end -->
        ...
        <?php
                 ...
                 $sector=$_POST['sector'];
                 $anio=$_POST['anio'];
                $sql="SELECT DISTINCT procedimiento.nombre_pro FROM procedimiento INNER JOIN acta ON acta.cod_pro=procedimiento.codigo_pro WHERE acta.sector_act='$sector' AND EXTRACT(YEAR FROM acta.fecha_act)='$anio'";
               
                 $datos=$obj->cantidad($sql);
                  if($datos<1){
                      echo "<script language='javascript'> alert('No hay Procedimientos en este Sector para este año!'); window.location.href='GraficasActasSector.php';</script>"; 
                    } 
                   else{
...
$(function () {
    $('#unico3').highcharts({
        ...
        title: {
            text: 'Estadísticas de Procedimientos en el Sector <?php echo $sector?> para el Año <?php echo $anio?> '
        },
        yAxis: {
            min: 0,
            title: {
        ...
        <?php
                 ...
                 while($fila = mysqli_fetch_array($datos)){
                      $fecha=$fila['fechanue'];
*/
                      $sql2="SELECT DISTINCT procedimiento.codigo_pro FROM procedimiento INNER JOIN acta ON acta.cod_pro=procedimiento.codigo_pro WHERE acta.sector_act='$sector' AND EXTRACT(YEAR FROM acta.fecha_act)='$anio'";
                      ...
                      while($fila2 = mysqli_fetch_array($datos2)){
                            ...
                            $nuepro=$fila2['codigo_pro'];
                               $sql3="SELECT procedimiento.nombre_pro FROM procedimiento INNER JOIN acta ON acta.cod_pro=procedimiento.codigo_pro WHERE acta.sector_act='$sector' AND procedimiento.codigo_pro='$nuepro' AND EXTRACT(YEAR FROM acta.fecha_act)='$anio'";
