<?php 
	...
	INNER JOIN Multa_Persona mp ON m.codigo_mul = mp.cod_mul INNER JOIN Persona p ON p.codigo_per = mp.cod_per WHERE m.status_mul = 'Inactiva' GROUP BY m.codigo_mul ORDER BY m.numero_mul  ";
	...
	if(!$resultado){
			echo "<script language='javascript'> alert('Error de Consulta!'); </script>";
				...
				if(!mysql_num_rows($resultado)){
					...
					echo "<script language='javascript'> alert('Ninguna Multa Inactiva Registrada!');  window.close();</script>";
